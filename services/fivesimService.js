...
const getAvailableCountries = async () => {
  ...
  const res = await fetch(`${baseUrl}/guest/countries`, {
    headers: { Accept: 'application/json' },
  });

  if (!res.ok) {
    const body = await res.text();
    throw new Error(`Failed to fetch countries from provider (${res.status}): ${body}`);
  }
...
const getAvailableProducts = async (country, operator = 'any') => {
  ...
  /*
    5sim can return:
      { service: { operatorName: { Price, Qty } } }
    OR:
      { service: { Price, Qty, Category } }
...
// ============================================================
// CANCEL ORDER
// FIX: 5sim may return plain text ("ACCESS_CANCEL") instead of
// JSON — parse defensively so the caller never gets a parse error.
// ============================================================
...
// ============================================================
// FINISH ORDER
// FIX: 5sim may return plain text ("ACCESS_READY") instead of
// JSON — parse defensively so the caller never gets a parse error.
// ============================================================
const finishOrder = async (orderId) => {
  ...
  if (!res.ok) {
    throw new Error(`Failed to finish order (${res.status}): ${text}`);
  }
